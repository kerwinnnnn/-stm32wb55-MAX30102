...
typedef struct{
  ...
  uint16_t  CustomDatasendHdle;                  /**< datasend handle */
...
#define COPY_DATASEND_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   *                                2 for datasend +
   *                                1 for datasend configuration descriptor +
   *                                1 for datasend broadcast property +
   *                              = 5
   *
  ...
  ret = aci_gatt_add_char(CustomContext.CustomMysvcHdle,
                          UUID_TYPE_128, &uuid,
                          SizeDatasend,
                          CHAR_PROP_BROADCAST | CHAR_PROP_READ | CHAR_PROP_WRITE | CHAR_PROP_NOTIFY | CHAR_PROP_INDICATE,
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_DATASEND:
      ret = aci_gatt_update_char_value(CustomContext.CustomMysvcHdle,
                                       CustomContext.CustomDatasendHdle,
                                       0, /* charValOffset */
                                       SizeDatasend, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value DATASEND command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value DATASEND command\n\r");
      }
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_DATASEND:
	  ...
	  Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomMysvcHdle, CustomContext.CustomDatasendHdle, SizeDatasend, pPayload);
